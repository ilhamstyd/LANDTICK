...
export const MyTicketPending = () => {
  ...
  let {data: transaction } = useQuery("transactioneCache", async () => {
    ...
    return(
        ...
        <>
          <Card className="ms-5 mt-3" style={{ width: "1035px", height: "344px"}} key={index}>
                
                <div>
                  <img className="position-absolute ms-4 my-auto" src="/images/Land.png" alt="Logo" />
                  <img src="/images/Vector.png" alt="Rectangle" />
                </div>
                <div className="position-absolute text-end" style={{ marginLeft: "820px" }}>
                  <h3 className="">Kereta Api</h3>
                  <p className=" fw-semibold text-secondary">sabtu, 21 Februari 2022</p>
                </div>
                <div className="d-flex">
                  ...
                  <div className="ms-3 mt-4">
                    <div>
                      <h5 className="fw-bold">{transactions?.ticket.start_time}</h5>
                      <h5 className="text-secondary">{transactions?.ticket.start_date}</h5>
                    </div>
                    <div className="mt-5">
                      <h5 className="fw-bold">{transactions?.ticket.arrival_time}</h5>
                      <h5 className="text-secondary">{transactions?.ticket.start_date}</h5>
                    </div>
                  </div>
                  <div className="ms-5 mt-4">
                    <div>
                      <h5 className="fw-bold">{transactions?.ticket.start_station.kota}</h5>
                      <h5 className="text-secondary">{transactions?.ticket.start_station.name}</h5>
                    </div>
                    <div className="mt-5">
                      <h5 className="fw-bold">{transactions?.ticket.end_station.kota}</h5>
                      <h5 className="text-secondary">{transactions?.ticket.end_station.name}</h5>
                    </div>
                  </div>
                </div>
